...
namespace FinalProjectDB1.Forms.UpdateForms
{
    public partial class UpdateStudent : Form
    {
        ...
        private void royalButton1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
               ...
               // {
                    ...
                    admin.UpdateStudent(Fname, lname, email, password, contact, cnic, city, usertype, regno, department, semester, activeStatus, date, id);
                    ShowStudent();
                    clear();
               // }
               // else
               // {
               //     MessageBox.Show("Student with these credential already Exist with same contact or email or cnic or this name ");
               // }
        ...
        {
            var con = Configuration.getInstance().getConnection();
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM [User] JOIN Authentication AS a ON a.UserID = [User].UserID JOIN Student as s ON s.UserID = [User].UserID WHERE  Contact = @Contact OR a.Email = @Email OR CNIC = @CNIC OR s.RegistrationNo = @RegNo", con);
            cmd.Parameters.AddWithValue("@RegNo", uregno.Text);
